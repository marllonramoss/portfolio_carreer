...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/contact", [messageLimiter, cooldownMiddleware], async (req: Request, res: Response) => {
    try {
      ...
      if (!method || !contactInfo || !name || !message) {
        return res.status(400).json({
      ...
      if (!webhookUrl) {
        console.error('N8N_CONTACT_WEBHOOK_URL not set');
        return res.status(503).json({
      ...
      const n8nResponse = await fetch(webhookUrl, {
        ...
        body: JSON.stringify([{
          method,
          contactInfo: method === 'whatsapp' ? sanitizedContactInfo.replace(/[^+\d]/g, '') : sanitizedContactInfo,
          name: sanitizedName,
      ...
      if (!n8nResponse.ok) {
        throw new Error(`Failed to send message to n8n: ${n8nResponse.statusText}`);
      }
